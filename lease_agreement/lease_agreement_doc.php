...
<section class="print-doc">	
	...
	<p class="paragraph-doc"> 
		...
		4.4. Платежи производятся с периодичностью раз в 
			...
			</span> и предсталены в графике лизинговых платежей - Приложение 2.<br>
		4.4. Внесение платы за 1-й период производится с 
			<span class='date'>
				<script type="text/javascript"> // преобразовываем период
					formatDate( <?php echo $year_ndate ?>, <?php echo $month_ndate ?>, <?php echo $day_ndate ?>);
				</script>
			</span><br>
		...
		4.7. Расчет комиссионного вознаграждения лизингодателя осуществляется от балансовой стоимости имущества. 
		Ставка комиссионного вознаграждения лизингодателя <?php echo $row_lease_agreement['STV'];?>% годовых. 
		...
		(Приложение 3), подписываемый надлежаще уполномоченными представителями 
